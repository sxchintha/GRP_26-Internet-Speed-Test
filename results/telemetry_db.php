 ...
 */
function insertSpeedtestUser($ip, $ispinfo, $extra, $ua, $lang, $dl, $ul, $ping, $jitter, $log, $user, $district, $latitude, $longitude)
{
    $pdo = getPdo();
    ...
    try {
        ...
        (ip,ispinfo,extra,ua,lang,dl,ul,ping,jitter,log,user,district,latitude,longitude)
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)'
        );
        $stmt->execute([
            $ip, $ispinfo, $extra, $ua, $lang, $dl, $ul, $ping, $jitter, $log, $user, $district, $latitude, $longitude]);
...
function getConnectionSpeedtest()
{
    $pdo = getPdo();
    if (!($pdo instanceof PDO)) {
        return false;
